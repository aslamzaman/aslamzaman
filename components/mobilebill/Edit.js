...
const Edit = ({ message, id }) => {
    const [mobile_id, setMobile_id] = useState('');
    ...
    const showEditForm = async () => {
        ...
        try {
            ...
            const mobileResponse = await fetchAll('mobile');
            const mobileData = mobileResponse.data;
            setMobiles(mobileData);

            const response = getOne("mobilebill", id);
            if (response) {
                setMobile_id(response.data.mobile_id);
    ...
    const saveHandler = (e) => {
        ...
        try {
            ...
            const response = updateItem("mobilebill", id, newObject);
