...
const Print = () => {
    ...
    useEffect(() => {
        try {
            ...
            setNames(nm);
            setSubject(subject);
            const detialData = detail.split(";");
            setDetails(detialData);
        } catch (error) {
            console.log(error);
        }
    }, [])
        ...
        <>
            <div className="px-4 lg:px-6">
                <div className="w-full py-6 flex justify-center items-center space-x-4 border-b border-gray-300 rounded-t-md">
                    ...
                    <ReactToPrint trigger={printButton} content={() => contentRef.current} pageStyle={pageStyle} documentTitle="Staff Appointment" />
                </div>
                ...
                <div className="w-full p-4 overflow-auto">
                    ...
                    <div className="w-[730px] h-fit p-[60px] border-2 border-gray-200 mx-auto">
                        <div id="page" ref={contentRef} className={`w-full h-auto bg-white ${tiro.className}`} >
                            ...
                            <p className="w-full text-justify">
                                বিষয়: <span className="font-bold">{subject}</span>
                            </p>
                            ...
                            <p className="w-full text-justify">
                                জনাব,<br />
                                {details.length > 0 ? details.map((detail, i) => <span key={i}>{detail}<br /><br /></span>) : null}
                            </p>
