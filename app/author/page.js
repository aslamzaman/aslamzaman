...
const Author = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL}/api/author`, {
                    method: "GET",
                                ...
                                authors.map(author => (
                                    <tr className="border-b border-gray-200 hover:bg-gray-100" key={author._id}>                                           
                                        ...
                                        <td className="h-8 flex justify-end items-center space-x-1 mt-1 mr-2">
                                            <Edit message={messageHandler} id={author._id} data={authors} />
                                            <Delete message={messageHandler} id={author._id} data={authors} />
                                        </td>
                                    </tr>
                                ))
                            ): (
