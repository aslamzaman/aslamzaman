...
export const fetchDataFromAPI = async (collectionName) => {
    try {
        ...
        const cacheTime = localStorage.getItem(`${collectionName}CacheTime`);
        const maxCacheAge = 60 * 60 * 1000; // 1 hour
        // const maxCacheAge = 1 * 60 * 1000; // 1 minute

        if (cachedData && cacheTime && (Date.now() - parseInt(cacheTime) < maxCacheAge)) {
        ...
        //Get Data Reading Length from localStorage
        // {tm:464646, len:6646}
        const readingLength = localStorage.getItem('readinglen');
        if (readingLength) {
            const parsedReadingLength = JSON.parse(readingLength);
            const savedDate = new Date(parsedReadingLength.tm);
            const todayDate = new Date();
            // Compare saved and current dates
            const saveDay = `${savedDate.getFullYear()}${savedDate.getMonth()}${savedDate.getDate()}`;
            const todayDay = `${todayDate.getFullYear()}${todayDate.getMonth()}${todayDate.getDate()}`;
            if (saveDay === todayDay) {
                const totalLength = parseInt(parsedReadingLength.len) + data.length;
                localStorage.setItem('readinglen', JSON.stringify({ tm: Date.now(), len: totalLength }));
            } else {
                localStorage.setItem('readinglen', JSON.stringify({ tm: Date.now(), len: data.length }));
            }
        } else {
            localStorage.setItem('readinglen', JSON.stringify({ tm: Date.now(), len: data.length }));
        }
 ...
 * @returns
 */
export const putDataToAPI = async (collectionName, id, data) => {
    try {
        const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/${collectionName}/${id}`;
        const response = await fetch(url, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify(data)
        ...
        const localData = JSON.parse(localStorage.getItem(collectionName));
        const indexOfItem = localData.findIndex(item => item._id === id);
        localData[indexOfItem] = { ...responseData, ...data };
        localStorage.setItem(collectionName, JSON.stringify(localData));
 ...
 * @returns
 */
export const patchDataToAPI = async (collectionName, id) => {
    try {
        const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/${collectionName}/${id}`;
        const response = await fetch(url, {
            method: "PATCH",
        ...
        const localData = JSON.parse(localStorage.getItem(collectionName));
        const indexOfItem = localData.findIndex(item => item._id === id);
        localData[indexOfItem] = { ...responseData, isDeleted: true };
        localStorage.setItem(collectionName, JSON.stringify(localData));
 ...
 * @returns 
 */
export const deleteDataFromAPI = async (collectionName, id) => {
    try {
        const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/${collectionName}/${id}`;
        const response = await fetch(url, {
            method: "DELETE",
        ...
        const localData = JSON.parse(localStorage.getItem(collectionName));
        const updatedItems = localData.filter(item => item._id !== id);
